...
var { getSlides, getSlidesFromOrderedList, generateSlidesHTML, generateSlidesFromSimpleHtml } = require("./slides");
var { REVEAL_FILES, HL_STYLES, INDEX,
  INIT_FILES, USER_FILES, DATA, MATHJAX_DIR } = require("./paths");
...
function build (from, to, options) {
  ...
  //Copy Mathjax all files
  copy(MATHJAX_DIR, path.join(to, "js"));
...
function buildIndex (from, to, options, addAssets) {
  ...
  var slides = options.slideList ? 
    getSlidesFromOrderedList(from, options) :
    getSlides(from, options);
  copy(path.join(DATA, INDEX), to);

  var generatedSlides;
  if (options.singleHtml)
    generatedSlides = generateSlidesFromSimpleHtml(slides);
  else
    generatedSlides = generateSlidesHTML(slides, options.subCovers);

  var injectAll = compose(
    inject(generatedSlides, "<!-- SLIDES HERE -->"),
    // meta
